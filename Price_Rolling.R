...
for(j in 1:length(myfiles)){
  ...
  fcmat <- matrix(0, nrow=n, ncol=14)    # empty matrix for forecast values
  ...
  for(i in 1:n){
    x_i <- subset(df, start=i, end=nrow(df)*0.5716 + i - 1)    # shift x by i
    xreg_i <- subset(df, start=nrow(df)*0.5716 + i)    # shift regressors by i
    lambda <- BoxCox.lambda(x_i[,"dam_price"])    # BoxCox transformation lambda for Training data
